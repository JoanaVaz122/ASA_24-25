...
void fill_square(int lin, int col, vector<vector<vector<int>>>& tabela,
    ...
    if (lin >= col || lin < 0 || col >= (int)tabela.size()) return; // Verificação de limites
    ...
    int full = 0;
    bool found = false;
    for (int k = col - 1; k >= lin; k--) { // Tenta todas as divisões
        for (int i : tabela[lin][k]) { // Valores à esquerda
            if (i == -1) break;
            for (int j : tabela[k + 1][col]) { // Valores à direita
                ...
                if (full < n && tabela[col][lin][res] == -1) {
                    ...
                    tabela[col][lin][res] = k;     // Índice da divisão
    ...
    // Marca como inválido se nenhuma solução válida foi encontrada
...
void reconstruct(int lin, int col, int v, const vector<vector<vector<int>>>& tabela, int n, string& result) {
    ...
    if (v < 0 || v >= n || lin < 0 || col >= (int)tabela.size() || tabela[col][lin][v] == -1) {
        result = "Invalid"; // Marcar explicitamente como inválido
        return;
    }
    ...
    if (result == "Invalid") return; // Parar se for inválido
    result += " ";
...
int main() {
    int n, m;
    if (scanf("%d %d", &n, &m) != 2) return 1; // Verificação de entrada
    ...
    for (int i = 0; i < m; ++i) {
        tabela[i][i][0] = sequence[i] - 1; // Ajustar índice para 0-based
    }
