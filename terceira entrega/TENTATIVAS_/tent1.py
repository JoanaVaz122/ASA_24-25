...
def solve_toy_distribution(input_data):
    ...
    problem = LpProblem("MaximizeToyDistribution", LpMaximize)
    ...
    problem += lpSum(x.values()), "Maximize total satisfied requests"
    ...
    for req in requests:  
        child_id = req[0]
        factories_list = req[2:]
        problem += lpSum(x[(child_id, factory_id)] for factory_id in factories_list) <= 1, f"OneToyPerChild_{child_id}"
